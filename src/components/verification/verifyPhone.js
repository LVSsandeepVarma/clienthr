  ...
  const handlePhoneVerification = async() => {
    ...
    try{
      if (phone && verifyPhone(phone)) {
        const response = await axios.post("https://hrmbackdoor.globalcrmsoftware.com/api/hrm-candidate/auth/verify-phone",{mobile:phone,token:window.location.pathname?.split("/candidate/verify/")[1]})
        ...
        <div className="w-full min-h-screen p-5 md:p-20 flex items-center justify-center">
          <div className="w-96 intro-y">
            ...
            <div className="text-white dark:text-slate-300 text-2xl font-medium text-center mt-14">
              {!phoneVerifiedStatus ? "Verify Your Phone Number": "OTP Verification"}
            </div>
            {!phoneVerifiedStatus ? (<div className="box px-5 py-8 mt-10 max-w-[450px] relative before:content-[''] before:z-[-1] before:w-[95%] before:h-full before:bg-slate-200 before:border before:border-slate-200 before:-mt-5 before:absolute before:rounded-lg before:mx-auto before:inset-x-0 before:dark:bg-darkmode-600/70 before:dark:border-darkmode-500/60">
              ...
              />
              {phoneRrror && <p className="!text-red-600 font-bold" style={{color:"red"}}>{phoneRrror}</p>}
              {apiErr?.length>0 && apiErr?.map((val,ind)=>{
                return(
                Object?.values(val).map((er,ind)=>(
                  <p className='font-bold' style={{color:"red"}}>{er}</p>
                )))
              })}
              {phoneSuccess && <p className="font-bold" style={{color:"green"}}>{phoneSuccess}</p>}
